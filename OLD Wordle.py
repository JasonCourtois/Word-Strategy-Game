...
def colored(r, g, b, text):  # ascii escape sequence code for color... idk how this works lol I copied it
...
class game:
    ...
    def row(self, num):
        print("--", end="")
        for l in range(len(self.boxes[num])):
            print(f" {self.boxes[num][int(l)]} ", end="")  # for loop prints 1 row of boxes based on num
        print("--")
    ...
    def checkInput(self, guess):
        ...
        for w in range(len(guess)):
            ...
            for match in re.finditer(guess[w], self.cword):
                if int(match.start()) == w and int(totl[str(guess[w])]) > 0:  # if pos match AND dict remaining
                    self.colors[w] = 2
                    yletter[w] = ' '  # resets yellow letter in this pos
                elif int(match.start()) != w and int(totl[str(guess[w])]) > 0:
                    if not self.colors[w] == 2:  # don't overwrite green color
                        self.colors[w] = 1
                        yletter[w] = str(guess[w])
                if int(match.start()) == w and int(totl[str(guess[w])]) <= 0:  # *FAILSAFE* if green match but dict =0
                    yletter = str("".join(yletter))  # change yletter to str
                    if re.search(guess[w], yletter) and yreplace == 1:
                        removey = re.search(guess[w], yletter).start()
                        for match2 in re.finditer(guess[w], yletter):
            ...
            if re.search(guess[w], self.cword) and replace == 1:
                totl[str(guess[w])] -= 1  # increment dict down by 1
                replace = 0

    def letterBank(self):  # print function for keyboard
        print("---Used Letters:---")
...
while again:
    ...
    while not wordle.checkWinner() and wordle.crow <= 5:  # max rows 5 (index start at 0)
        wordle.printBoard()
        wordle.letterBank()
        ...
        wordle.checkInput(guess)
        for l in range(len(guess)):
            pos = re.search(guess[l], kboard).start()  # gets index for keyboard
            if wordle.colors[l] == 0:  # if the current color val of letter is 0, print as grey
                wordle.boxes[wordle.crow][l] = colored(180, 180, 180, guess[l])
            ...
            elif wordle.colors[l] == 1:  # if the current color val of letter is 1, print as yellow
                wordle.boxes[wordle.crow][l] = colored(255, 255, 0, guess[l])
            ...
            elif wordle.colors[l] == 2:  # if the current color val of letter is 2, print as green
                wordle.boxes[wordle.crow][l] = colored(0, 255, 0, guess[l])
                pboard[pos] = colored(0, 255, 0, guess[l])
                kcolors[guess[l]] = "2"  # green can override all colors as it is the most prominent
        wordle.crow += 1  # increment to next row
        clear()

    clear()
    wordle.printBoard()
    wordle.letterBank()
    if wordle.checkWinner():
        print("------You guessed the word!------")
    else:
        print(f"-----The word was: {wordle.cword}, nice try!-----")
